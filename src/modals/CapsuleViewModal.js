...
const CapsuleViewModal = ({ visible, onClose, userId }) => {
  ...
  const handleCapsulePress = (capsule) => {
    // Kapsül detaylarını göster
    console.log("Kapsül seçildi:", capsule.id);
  };

  const handleViewCapsule = (capsule) => {
    // Açılmış kapsülün içeriğini görüntüle
    console.log("Kapsül içeriği görüntüleniyor:", capsule.id);
  };
